                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-10">
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center gap-4 cursor-pointer group" onClick={() => navigate('/')}>
                            <img src={settings?.logoUrl || "/logo.png"} alt="Dwarika Hospital Logo" className="w-14 h-14 object-contain brightness-0 invert opacity-90 group-hover:opacity-100 transition-all" />
                            <div>
                                <h4 className="text-lg font-bold text-white leading-tight">
                                    {settings?.brandNameEn || 'Dwarika Hospital'}
                                </h4>
                                <p className="text-xs text-slate-300">{settings?.brandSubtitle || '& Medical Academy Pvt. Ltd.'}</p>
                            </div>
                        </div>
                        <p className="text-sm leading-relaxed" style={{ color: 'rgba(255, 255, 255, 0.85)' }}>
                            {settings?.footerText || t('footer.brand_desc')}
                        </p>
                        <div className="flex gap-2">
                            {[
                                { icon: Facebook, href: settings?.social?.facebook || "https://www.facebook.com/61573322175633/", hoverBg: "hover:bg-[#1877F2]" },
                                { icon: Instagram, href: settings?.social?.instagram || "https://www.instagram.com/dwarikahospital15/", hoverBg: "hover:bg-[#E4405F]" },
                                { 
                                    icon: () => (
                                    ...
                                    href: settings?.social?.whatsapp || "https://wa.me/9779705490123",
                                    hoverBg: "hover:bg-[#25D366]"
                                },
                                { icon: Linkedin, href: settings?.social?.linkedin || "https://linkedin.com", hoverBg: "hover:bg-[#0A66C2]" },
                        ...
                        <ul className="space-y-4">
                            ...
                            <li className="flex gap-3 items-start group">
                                ...
                                <div className="flex flex-col gap-1 mt-1">
                                    <a href={`tel:${(settings?.landline || '031590123').replace(/\D/g, '')}`} className="text-sm text-slate-100 hover:text-white transition-colors">{settings?.landline || '031 - 590123'}</a>
                                    {settings?.mobile ? (
                                        <a href={`tel:${settings.mobile.replace(/\D/g, '')}`} className="text-sm text-slate-100 hover:text-white transition-colors">{settings.mobile}</a>
                                    ) : (
                            ...
                            <li className="flex items-center gap-3 group">
                                ...
                                <a href={`mailto:${settings?.emails?.info || settings?.emails?.general || 'info@dhama.com.np'}`} className="text-sm text-slate-100 hover:text-white transition-colors">{settings?.emails?.info || settings?.emails?.general || 'info@dhama.com.np'}</a>
                            </li>
                            <li className="flex items-center gap-3 group">
                                <div className="p-2 bg-slate-800/90 rounded-lg ring-1 ring-white/8 group-hover:bg-primary transition-colors shrink-0">
                                    <Globe className="w-4 h-4 text-slate-100 group-hover:text-white" />
                                </div>
                                <a href={settings?.websiteUrl || 'https://www.dhama.com.np'} target="_blank" rel="noreferrer" className="text-sm text-slate-100 hover:text-white transition-colors">{(settings?.websiteUrl || 'www.dhama.com.np').replace(/^https?:\/\//, '')}</a>
                            </li>
                        </ul>
                    </div>
                </div>
