                ...
                <>
                    ...
                    >
                        ...
                        <div className="flex-1 overflow-auto px-10 pb-10">
                            <div className="rounded-[32px] border border-slate-200 overflow-hidden bg-slate-50/30 shadow-inner">
                                <table className="w-full border-collapse">
                                    <thead>
                                        <tr className="bg-white">
                                            <th className="p-6 text-left border-b border-r border-slate-200 w-[200px]">
                                                <span className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">SHIFT</span>
                                            </th>
                                            {days.map((day, idx) => (
                                                <th key={idx} className="p-6 text-center border-b border-r border-slate-200 last:border-r-0">
                                                    <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1.5 opacity-60">{day.adDayName}</p>
                                                    <p className="text-xl font-black text-blue-950 uppercase tracking-tight">
                                                        DAY {calendarMode === 'BS' ? day.bsDayNum : day.adDayNum}
                                                    </p>
                                                </th>
                                            ))}
                                        </tr>
                                            ...
                                            <tr key={shift.id} className="bg-white">
                                                ...
                                                {days.map((day, dIdx) => {
                                                    ...
                                                    return (
                                                        <td 
                                                            key={dIdx} 
                                                            className={`p-2 border-b border-r border-slate-200 last:border-r-0 transition-all relative
                                                                ${!time ? 'bg-slate-50/50' : 'cursor-pointer hover:bg-blue-50/30'}
                                                            `}
                                                            onClick={() => time && setSelectedSlot({ dayIdx: dIdx, shiftId: shift.id, time })}
                                                        >
                                                            <div className={`
                                                                relative h-full min-h-[90px] rounded-2xl flex flex-col items-center justify-center gap-3 transition-all duration-300
                                                                ${isSelected ? 'bg-blue-600 shadow-xl shadow-blue-600/20 ring-4 ring-blue-100 scale-[0.98]' : ''}
                                                                ${!time ? 'opacity-30' : 'group'}
                                                            `}>
                                                                {time ? (
                                                                    <>
                                                                        <span className={`
                                                                            text-[11px] font-black px-4 py-2 rounded-xl transition-all border
                                                                            ${isSelected 
                                                                                ? 'bg-white/10 text-white border-white/20' 
                                                                                : 'bg-white text-blue-900 border-blue-100 shadow-sm group-hover:scale-105 group-hover:shadow-md'
                                                                            }
                                                                        `}>
                                                                            {time}
                                                                        </span>
